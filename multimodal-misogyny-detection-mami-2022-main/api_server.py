...
model.load_state_dict(torch.load("saved_models/model_resnet50.pt", map_location=torch.device('cpu')))
model.to(device)
...
def test_function_call():
    return jsonify({"mesaj": "acesta e un mesaj de test!"})
...
def upload_image_and_text():
    try:
        ...
        text = request.form['text']  # Retrieve the text stringcitadel
