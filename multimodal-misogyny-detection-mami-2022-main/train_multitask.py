...
def train(model, optimizer, lr_scheduler, num_epochs):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for img_inps, txt_tokens, masks, labels1, labels2, labels3, labels4, labels5 in tr_loader:
            ...
            loss = criterion(outputs1, labels1.unsqueeze(1).float()) + criterion(outputs2,
                                                                                 labels2.unsqueeze(1).float()) + \
                   criterion(outputs3, labels3.unsqueeze(1).float()) + criterion(outputs4,
                                                                                 labels4.unsqueeze(1).float()) + \
...
def evaluate(model, loader):
    ...
    with torch.no_grad():
        for img_inps, txt_tokens, masks, labels1, labels2, labels3, labels4, labels5 in loader:
            ...
            test_loss += (criterion(outputs1, labels1.unsqueeze(1).float()).item() + criterion(outputs2,
                                                                                               labels2.unsqueeze(
                                                                                                   1).float()).item() + \
                          criterion(outputs3, labels3.unsqueeze(1).float()).item() + criterion(outputs4,
                                                                                               labels4.unsqueeze(
                                                                                                   1).float()).item() + \
